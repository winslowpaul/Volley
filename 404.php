...
$enable_particles = themethreads_helper()->get_option( 'error-404-enable-particles' );
		...
		<div class="row">	
			<div class="col-md-8 col-md-offset-2 text-center">
				...
				<div class="text-404">
					...
					<h1 data-fittext="true" data-fittext-options='{ "compressor": 0.25, "minFontSize": 150, "maxFontSize": 300 }' class="themethreads-counter-element" data-enable-counter="true" data-counter-options='{ "targetNumber": "404", "blurEffect": true }'>
						<!--/.THIS IS NOT TRANSLATABLE OR DYNAMIC THING, IT NEEDS FOR THE EFFECTS -->
						<span>000</span>
						
					</h1><!-- /.themethreads-counter-element -->
				...
				<?php if( !class_exists( 'ReduxFramework' ) ) : ?>
					...
					<h3 class="font-weight-bold mb-1"><?php esc_html_e( 'Looks like you are lost.', 'volley' ); ?></h3>
					<p><?php esc_html_e( 'We can’t seem to find the page you’re looking for.', 'volley' ) ?></p>					
					<a href="<?php echo esc_url( home_url('/') ) ?>" class="btn btn-md btn-solid btn-gradient circle btn-icon-left font-weight-bold text-uppercase ltr-sp-1 wide">
						<span>
							...
							<span class="btn-txt"><?php esc_html_e( 'Go Home!', 'volley' ); ?></span>
				...
				<?php else : ?>
					...
					<h3 class="font-weight-bold mb-1"><?php themethreads_helper()->get_option_echo( 'error-404-subtitle', 'html', '', 'options' ) ?></h3>
					<?php echo apply_filters( 'the_content', themethreads_helper()->get_option( 'error-404-content', 'post', '', 'options' ) ) ?>
					<?php if( 'on' === themethreads_helper()->get_option( 'error-404-enable-btn', 'raw', '', 'options' ) ) { ?>
						<a href="<?php echo esc_url( home_url('/') ) ?>" class="btn btn-md btn-solid btn-gradient circle btn-icon-left font-weight-bold text-uppercase ltr-sp-1 wide">
						<span>
							<span class="btn-gradient-bg"></span>
							<span class="btn-icon">
							...
							<span class="btn-txt"><?php themethreads_helper()->get_option_echo( 'error-404-btn-title', 'html', '', 'options' ) ?></span>
							<span class="btn-gradient-bg btn-gradient-bg-hover"></span>
						</span>
					</a>
					<?php } ?>
