...
* The ThemeThreads_Admin initiate the theme admin
*/

if( !defined( 'ABSPATH' ) )
	exit; // Exit if accessed directly

class ThemeThreads_Admin extends ThemeThreads_Base {
	...
	public function __construct() {
		...
		// Envato Market
		get_template_part( 'themethreads/libs/importer/themethreads', 'importer' );
	...
	public function init() {
		...
		themethreads()->load_theme_part( 'themethreads-register-plugins' );

		include_once( get_template_directory() . '/themethreads/admin/themethreads-admin-page.php' );
		include_once( get_template_directory() . '/themethreads/admin/themethreads-admin-dashboard.php' );
		include_once( get_template_directory() . '/themethreads/admin/themethreads-admin-plugins.php' ) ;
		include_once( get_template_directory() . '/themethreads/admin/themethreads-admin-import.php' );
	}
    ...
    public function enqueue() {
		...
		wp_enqueue_style( 'font-awesome', themethreads()->load_assets( 'css/font-awesome.min.css' ) );
		...
		if( 'nav-menus.php' == $pagenow || 'widgets.php' == $pagenow ) {
			//iconpicker
			wp_enqueue_style( 'themethreads-icon-picker-main-css', themethreads()->load_assets( 'vendors/iconpicker/css/jquery.fonticonpicker.min.css' ) );
			wp_enqueue_style( 'themethreads-icon-picker-main-css-theme', themethreads()->load_assets( 'vendors/iconpicker/themes/grey-theme/jquery.fonticonpicker.grey.min.css' ) );
		}

		//imagepicker
		wp_enqueue_style( 'themethreads-imagepicker-css', themethreads()->load_assets( 'vendors/image-picker/image-picker.css' ) );
		wp_enqueue_style( 'jquery-confirm-css', themethreads()->load_assets( 'css/jquery-confirm.min.css' ) );

		if( 'nav-menus.php' == $pagenow || 'widgets.php' == $pagenow ) {
			wp_enqueue_script( 'themethreads-icon-picker', themethreads()->load_assets( 'vendors/iconpicker/jquery.fonticonpicker.min.js' ), array( 'jquery' ), false, true );
			wp_enqueue_script( 'themethreads-custom-icon-upload', themethreads()->load_assets( 'js/themethreads-custom-icon-upload.js' ), array( 'jquery' ), false, true );
			wp_localize_script(
				'themethreads-custom-icon-upload', 'themethreadsMenuCustomIcon', array(
					'l10n'     => array(
						'uploaderTitle'      => esc_html__( 'Choose an image/svg icon', 'volley' ),
						'uploaderButtonText' => esc_html__( 'Select', 'volley' ),
					),
					'settings' => array(
		...
		wp_enqueue_style( 'threads-dashboard', themethreads()->load_assets( 'css/themethreads-dashboard.min.css' ) );
		...
		wp_enqueue_script( 'themethreads-image-picker', themethreads()->load_assets( 'vendors/image-picker/image-picker.min.js' ), array( 'jquery' ), false, true );
		wp_enqueue_script( 'jquery-confirm', themethreads()->load_assets( 'js/jquery-confirm.min.js' ), array( 'jquery' ), false, true );
		wp_enqueue_script( 'themethreads-admin', themethreads()->load_assets( 'js/themethreads-admin.js' ), array( 'jquery', 'intersection-observer', 'underscore', 'themethreads-image-picker' ), false, true );
		wp_localize_script( 'themethreads-admin', 'themethreads_admin_messages', array(
			'reset_title'     => wp_kses_post( __( '<span class="dashicons dashicons-info"></span> Reset', 'volley' ) ),
			'reset_message'   => esc_html__( 'Remove posts, pages, media and any other content on your current site, We strongly recommend to reset before importing ( even if this is a fresh site ) to avoid any overlap or conflict with your current content.<br/><strong>Note:</strong> Don\'t use the reset option if you are trying to import some parts only ( For example if you are going to import theme options only then you may continue without reset )', 'volley' ),
			'reset_confirm'   => esc_html__( 'Reset Then Import', 'volley' ),
			'reset_continue'  => esc_html__( 'Keep Importing Without Resetting', 'volley' ),
			'reset_final_confirm' => esc_html__( 'I understand', 'volley' ),
			'reset_final_title'   => wp_kses_post( __( '<span class="dashicons dashicons-warning"></span> Warning', 'volley' ) ),
			'reset_final_message' => esc_html__( 'Since you selected to reset before importing please be aware this action cannot be reversed ( Any removed content cannot be restored )', 'volley' )
		) );

		// Icons
		$uri = get_template_directory_uri() . '/assets/vendors/' ;
		wp_register_style('themethreads-icons', $uri . 'themethreads-icon/themethreads-icon.min.css' );
	...
	public function vc_frontend_editor_js() {
		...
		$this->script( 'jquery-throttle-debounce', $this->get_vendor_uri( 'jquery.ba-throttle-debounce.min.js' ), array( 'jquery' ) );
		...
		$this->script( 'themethreads-mailchimp-form', $this->get_js_uri( 'mailchimp-form' ), array( 'jquery' ) );
		wp_localize_script( 'themethreads-mailchimp-form', 'ajax_themethreads_mailchimp_form_object', array(
		...
		// At the End
		$this->script( 'themethreads-theme', $this->get_js_uri( 'theme.min' ), $deps );
		wp_enqueue_script( 'themethreads-theme' );
	}

	public function vc_iconpicker_editor_jscss() {

		$font_icons = themethreads_helper()->get_theme_option( 'font-icons' );
		if( !empty( $font_icons ) ) {
			...
			wp_enqueue_style( 'themethreads-icons' );
		}
	}
	...
	public function admin_redirects() {
		...
		if ( is_admin() && 'themes.php' == $pagenow && isset( $_GET['activated'] ) ) {
			wp_redirect( admin_url( 'admin.php?page=themethreads' ) );
			exit;
		}
	}
	...
	public function default_header_content( $content, $post ) {
	    ...
	    if( 'themethreads-header' !== $post_type ) {
		    return $content;
		}
    ...
    public function vc_header_elements_tabs( $tabs ) {
	    ...
	    $tabs = array(
			array(
				'name' => esc_html__( 'Header Modules', 'volley' ),
				'active' => false,
	...
	public function fix_parent_menu() {
		...
		$submenu['themethreads'][0][0] = esc_html__( 'Dashboard', 'volley' );
	...
	public function save_plugins() {
		...
		// Deactivate Plugin
        if ( isset( $_GET['themethreads-deactivate'] ) && 'deactivate-plugin' == $_GET['themethreads-deactivate'] ) {

			check_admin_referer( 'themethreads-deactivate', 'themethreads-deactivate-nonce' );
		...
		// Activate plugin
		if ( isset( $_GET['themethreads-activate'] ) && 'activate-plugin' == $_GET['themethreads-activate'] ) {

			check_admin_referer( 'themethreads-activate', 'themethreads-activate-nonce' );
