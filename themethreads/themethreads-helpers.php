 ...
 *
 * @class ThemeThreads_Helper
*/
class ThemeThreads_Helper {

	/**
	 * Hold an instance of ThemeThreads_Helper class.
	 * @var ThemeThreads_Helper
	 */
	protected static $instance = null;

	/**
	 * Main ThemeThreads_Helper instance.
	 *
	 * @return ThemeThreads_Helper - Main instance.
	 */
	public static function instance() {

		if(null == self::$instance) {
			self::$instance = new ThemeThreads_Helper();
		}
	...
	public function get_template_part( $template, $args = null ) {
		...
		if ( ! file_exists( $located ) ) {
			_doing_it_wrong( __FUNCTION__, sprintf( wp_kses_post( __( '<code>%s</code> does not exist.', 'volley' ) ), $located ), null );
			return;
		}
	...
	public function tgmpa_plugin_action( $plugin, $status ) {
		...
		switch( $status ) {
			case 'not-installed':
				$btn_class = 'white';
				$btn_text = esc_html_x( 'Install', 'ThemeThreads plugin installation page.', 'volley' );
			...
			case 'installed':
				$btn_class = 'success';
				$btn_text = esc_html_x( 'Activate', 'ThemeThreads plugin installation page.', 'volley' );

				$nonce_url = wp_nonce_url(
					add_query_arg(
						array(
							'plugin' => urlencode( $plugin['slug'] ),
							'themethreads-activate' => 'activate-plugin'
						),
						$page
					),
					'themethreads-activate',
					'themethreads-activate-nonce'
				);
				break;

			case 'active':
				$btn_class = 'danger';
				$btn_text = esc_html_x( 'Deactivate', 'ThemeThreads plugin installation page.', 'volley' );

				$nonce_url = wp_nonce_url(
					add_query_arg(
						array(
							'plugin' => urlencode( $plugin['slug'] ),
							'themethreads-deactivate' => 'deactivate-plugin'
						),
						$page
					),
					'themethreads-deactivate',
					'themethreads-deactivate-nonce'
				);
				break;
		}

		printf(
			'<a class="themethreads-button" href="%4$s" title="%2$s %1$s"><span>%2$s</span> <i class="fa fa-angle-right"></i></a>',
	...
	public function attr( $context, $attributes = array() ) {
		...
		echo apply_filters( 'themethreads_attributes', $atts );
	}
	...
	public function get_attr( $context, $attributes = array() ) {
		...
		$attributes = apply_filters( "themethreads_attr_{$context}", $attributes, $context );

		$output = $this->html_attributes( $attributes );
	    $output = apply_filters( "themethreads_attr_{$context}_output", $output, $attributes, $context );
    ...
    public function get_option_echo( $id, $esc = 'raw', $default = false, $context = 'all' ) {
		...
		echo apply_filters( 'themethreads_get_option_echo', $option_value );
    }
	...
	public function get_theme_option( $id ) {
		...
		$options = $GLOBALS[themethreads()->get_option_name()];
	...
	public function dashboard_page_url() {
		...
		return admin_url( 'admin.php?page=themethreads' );
	}
	...
	public function plugin_page_url() {
		return admin_url( 'admin.php?page=themethreads-plugins' );
	}
	...
	public function import_demos_page_url() {
		return admin_url( 'admin.php?page=themethreads-import-demos' );
	}
...
/**
 * Main instance of ThemeThreads_Helper.
 *
 * Returns the main instance of ThemeThreads_Helper to prevent the need to use globals.
 *
 * @return ThemeThreads_Helper
 */
function themethreads_helper() {
	return ThemeThreads_Helper::instance();
}
