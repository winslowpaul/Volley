	...
	$(document).ready(function () {
		var menuImageUpdate = function( item_id, thumb_id, is_hover ) {
			wp.media.post( 'set-menu-item-thumbnail', {
				...
				_wpnonce:     themethreadsMenuCustomIcon.settings.nonce
			}).done( function( html ) {
		...
		$('#menu-to-edit')
			.on('click', '.menu-item .set-post-thumbnail', function (e) {
				...
				var item_id = $(this).parents('.field-image').siblings('input.menu-item-data-db-id').val(),
					...
					uploader = wp.media({
						title: themethreadsMenuCustomIcon.l10n.uploaderTitle, // todo: translate
						button: { text: themethreadsMenuCustomIcon.l10n.uploaderButtonText },
						multiple: false
					}).on('select', function () {
