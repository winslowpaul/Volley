...
* The ThemeThreads_Responsive_CSS initiate dynamic css.
*/

if( !defined( 'ABSPATH' ) )
	exit; // Exit if accessed directly

class ThemeThreads_Responsive_CSS extends ThemeThreads_Base {
	...
	public function can_write() {
		...
		$file_name   = '/themethreads-responsive-' . $version . '.css';
	...
	public function check_the_value() {
		...
		$the_value       = themethreads_helper()->get_option( 'media-mobile-nav' );
		$old_value       = get_option( 'themethreads_responsive_value', array() );
		$site_layout     = themethreads_helper()->get_option( 'page-layout' );
		$old_site_layout = get_option( 'themethreads_page_layout', array() );
		$site_width      = themethreads_helper()->get_option( 'site-width' );
		$old_site_width  = get_option( 'themethreads_site_width', array() );
		
		if( $the_value === $old_value && $site_layout === $old_site_layout && $site_width === $old_site_width ) {
			return false;
		}
		else {
			update_option( 'themethreads_responsive_value', $the_value );
			update_option( 'themethreads_page_layout', $site_layout );
			update_option( 'themethreads_site_width', $site_width );
			return true;
		}
	}
	...
	public function responsive_css() {
		 ...
		 */
		$responsive_css = themethreads_responsive_css();
	...
	public function enqueue_responsive_css() {
		...
		wp_enqueue_style( 'themethreads-generated-responsive', $this->file( 'uri' ) );
