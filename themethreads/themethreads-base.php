...
class ThemeThreads_Base {
	...
	public function load_extension( $name = '' ) {
		...
		if( !$located ) {
			wp_die( sprintf( wp_kses_post( __( '<strong>Extension Load Fail for:</strong> %1$s<br>No such file "%2$s" exists.', 'volley' ) ), strtoupper( $name ), "themethreads/extensions/{$name}/themethreads-{$name}.php" ), esc_html__( 'Load Extension', 'volley' ), null );
			return;
		}
...
if( ! function_exists( 'themethreads_action' ) ) :
	function themethreads_action() {
