...
class ThemeThreads_Mega_Menu_Walker extends Walker_Nav_Menu {
	...
	function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		if( !empty( $item->themethreads_megaprofile ) ) {
			$is_fullwidth = get_post_meta( $item->themethreads_megaprofile, 'megamenu-fullwidth', true );
			if( 'yes' === $is_fullwidth ) {
		...
		if( 'submenu-dark' === $item->themethreads_submenu_color ) {
			$item->classes[] = 'submenu-dark';
		}
		elseif( 'nav-item-children-style2' === $item->themethreads_submenu_color ) {
			$item->classes[] = 'nav-item-children-style2';
		}
		
		if( $item->thumbnail_id ) {
			$item->themethreads_icon = '';
			...
			if( 'svg' === $filetype['ext'] ) {
				...
				$custom_icon = wp_get_attachment_image( $item->thumbnail_id, 'full', false, array( 'class' => 'themethreads-custom-image-icon' ) );
			}
			
			if( 'left' === $item->themethreads_icon_position ) {
				$args->old_link_before = $args->link_before;
				$args->link_before = '<span class="link-icon left-icon">' . $custom_icon . '</span>' . $args->link_before;
			} 
			elseif( 'center' === $item->themethreads_icon_position ) {
		...
		if( ! empty( $item->themethreads_icon ) ) {
			if( 'left' === $item->themethreads_icon_position ) {
				$args->old_link_before = $args->link_before;
				$args->link_before = '<span class="link-icon left-icon"><i class="'. esc_attr( $item->themethreads_icon ) .'"></i></span>' . $args->link_before;
			} 
			elseif( 'center' === $item->themethreads_icon_position ) {
				$item->classes[] = 'center-icon';
				$args->old_link_before = $args->link_before;
				$args->link_before = '<span class="link-icon center-icon"><i class="'. esc_attr( $item->themethreads_icon ) .'"></i></span>' . $args->link_before;
			}
			else {
				$args->old_link_after = $args->link_after;
				$args->link_after = $args->link_after . '<span class="link-icon right-icon"><i class="'. esc_attr( $item->themethreads_icon ) .'"></i></span>';				
			}
		}

		if( !empty( $args->caret_visibility ) && ( $item->hasChildren || !empty( $item->themethreads_megaprofile ) ) ) {
			$args->old_link_after = $args->link_after;
			$args->link_after = $args->link_after . '<i class="fa fa-caret-down"></i>';
		}

		if( !empty( $item->themethreads_badge ) ) {
			$args->old_link_after = $args->link_after;
			$args->link_after = $args->link_after . '<mark class="'. esc_attr( $item->themethreads_badge_style ) .'">'. esc_html( $item->themethreads_badge ) . '</mark>';
		}
		...
		if( 'yes' === $item->themethreads_heading_item ) {
		...
		if( !empty( $item->themethreads_megaprofile ) ) {
			$item_html .= $this->get_megamenu( $item->themethreads_megaprofile );
		}
	...
	function get_megamenu( $id ) {
		...
		$css = themethreads_helper()->get_vc_custom_css( $id );
