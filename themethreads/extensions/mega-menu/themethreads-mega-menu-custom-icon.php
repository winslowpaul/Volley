...
* The ThemeThreads_Menu_Item_Custom_Image initiate the theme option machine.
*/

if( ! defined( 'ABSPATH' ) )
	exit; // Exit if accessed directly

class ThemeThreads_Menu_Item_Custom_Image {
	...
	public function menu_image_nav_menu_manage_columns( $columns ) {
		return $columns + array( 'image' => esc_html__( 'Image', 'volley' ) );
	}
	...
	public function wp_post_thumbnail_only_html( $item_id ) {
		...
		$content      = sprintf(
			$markup,
			esc_html__( 'Custom Image/svg Icon', 'volley' ),
			$thumbnail_id ? esc_attr__( 'Change menu item image/svg icon', 'volley' ) : esc_attr__( 'Set menu item image/svg icon', 'volley' ),
			'',
			$item_id,
			$thumbnail_id ? wp_get_attachment_image( $thumbnail_id, $default_size ) : esc_html__( 'Set image/svg icon', 'volley' ),
			$thumbnail_id ? '<a href="#" class="remove-post-thumbnail">' . esc_html__( 'Remove', 'volley' ) . '</a>' : ''
		);

		/*
		$hover_id = get_post_meta( $item_id, 'themethreads_thumbnail_hover_id', true );
		$content .= sprintf(
			$markup,
			esc_html__( 'Image on hover', 'volley' ),
			$hover_id ? esc_attr__( 'Change menu item image on hover', 'volley' ) : esc_attr__( 'Set menu item image on hover', 'volley' ),
			' hover-image',
			$item_id,
			$hover_id ? wp_get_attachment_image( $hover_id, $default_size ) : esc_html__( 'Set image on hover', 'volley' ),
			$hover_id ? '<a href="#" class="remove-post-thumbnail hover-image">' . esc_html__( 'Remove', 'volley' ) . '</a>' : ''
		);
		*/
...
new ThemeThreads_Menu_Item_Custom_Image;