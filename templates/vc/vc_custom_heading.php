...
// Additional Params
$letter_spacing = $gradient_color = $enable_fittext = $fittex_size = '';
...
if( 'yes' === $enable_fittext ) {
	$data_opts[] = 'data-fitText="true"';
	$data_opts[] = ! empty( $fittex_size ) ? 'data-max-fontSize="' . $fittex_size . '"' : '';
}
...
if( ! empty( $gradient_color ) ) {
	$styles[] = 'background:' . esc_attr( $gradient_color ) . '';
	$css_class .= ' vc_custom_heading_gradient ';
}
...
if ( apply_filters( 'vc_custom_heading_template_use_wrapper', false ) ) {
	$output .= '<div class="' . esc_attr( $css_class ) . '" ' . implode( ' ', $wrapper_attributes ) . ' ' . implode( ' ', $data_opts ) . '>';
	$output .= '<' . $font_container_data['values']['tag'] . ' ' . $style . ' >';
	$output .= $text;
	$output .= '</' . $font_container_data['values']['tag'] . '>';
	$output .= '</div>';
} else {
	$output .= '<' . $font_container_data['values']['tag'] . ' ' . $style . ' class="' . esc_attr( $css_class ) . '" ' . implode( ' ', $wrapper_attributes ) . ' ' . implode( ' ', $data_opts ) . '>';
	$output .= $text;
	$output .= '</' . $font_container_data['values']['tag'] . '>';
}
echo apply_filters( 'themethreads_vc_custom_heading', $output );