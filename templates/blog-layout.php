...
if( !class_exists( 'LD_Blog' ) ) {
	...
	if( !empty( $links ) ) {
		...
		printf( '<div class="blog-nav"><nav aria-label="%s"><ul class="pagination"><li>%s</li></ul></nav></div>', esc_attr__( 'Page navigation', 'volley' ), join( "</li>\n\t<li>", $links ) );
	}

	return;
}

get_template_part( 'theme/themethreads-blog' );