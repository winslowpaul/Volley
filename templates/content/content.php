...
<article <?php themethreads_helper()->attr( 'post' ) ?>>
	...
	<figure class="post-image hmedia themethreads-lp-media w-auto">
	...
	<?php if( is_singular() ) : ?>
		...
		<header class="entry-header">
			...
			<?php the_title( '<h1 '. themethreads_helper()->get_attr( 'entry-title' ) .'>', '</h1>' ); ?>

			<?php get_template_part( 'templates/entry', 'meta' ) ?>

		</header>

		<div <?php themethreads_helper()->attr( 'entry-content' ) ?>>
			<?php
				the_content( sprintf(
		...
		<footer class="entry-footer themethreads-lp-footer  d-flex flex-row flex-wrap">

			<?php themethreads_post_terms( array( 'taxonomy' => 'category', 'text' => esc_html__( 'Posted in: %s', 'volley' ) ) ); ?>
	...
	<?php else: ?>
		...
		<header class="entry-header themethreads-lp-header">
		...
		<footer class="entry-footer themethreads-lp-footer">
			<?php 
