...
<div class="threads-woo-steps">
	<div class="threads-woo-steps-inner">
		...
		<div class="threads-woo-steps-item">
...
<table class="shop_table shop_table_responsive cart" cellspacing="0">
	<thead>
		<tr>
			<th class="product-name" colspan="3"><?php esc_html_e( 'Product', 'volley' ); ?> <?php echo '('.count(WC()->cart->get_cart()).')'; ?></th>
			<th class="product-price"><?php esc_html_e( 'Price', 'volley' ); ?></th>
			<th class="product-quantity"><?php esc_html_e( 'Quantity', 'volley' ); ?></th>
			<th class="product-subtotal"><?php esc_html_e( 'Total', 'volley' ); ?></th>
			<th class="product-remove"><?php esc_html_e( 'Remove', 'volley' ); ?></th>
		</tr>
	</thead>
	<tbody>
		...
		foreach ( WC()->cart->get_cart() as $cart_item_key => $cart_item ) {
			...
			if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
				...
				<tr class="woocommerce-cart-form__cart-item <?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
					...
					<td class="product-name" data-title="<?php esc_attr_e( 'Product', 'volley' ); ?>" colspan="3">
						<span class="product-name-inner">
							...
							<?php
								...
								if ( $_product->backorders_require_notification() && $_product->is_on_backorder( $cart_item['quantity'] ) ) {
									echo wp_kses_post( apply_filters( 'woocommerce_cart_item_backorder_notification', '<p class="backorder_notification">' . esc_html__( 'Available on backorder', 'volley' ) . '</p>' ) );
								}
							?>
							</span>
						</span>
					...
					<td class="product-price" data-title="<?php esc_attr_e( 'Price', 'volley' ); ?>">
						<?php
							echo apply_filters( 'woocommerce_cart_item_price', WC()->cart->get_product_price( $_product ), $cart_item, $cart_item_key );
						?>
					</td>

					<td class="product-quantity" data-title="<?php esc_attr_e( 'Quantity', 'volley' ); ?>">
						<?php
					...
					<td class="product-subtotal" data-title="<?php esc_attr_e( 'Total', 'volley' ); ?>">
						<?php
							echo apply_filters( 'woocommerce_cart_item_subtotal', WC()->cart->get_product_subtotal( $_product, $cart_item['quantity'] ), $cart_item, $cart_item_key );
						?>
					</td>
					<td class="product-remove" data-title="<?php esc_attr_e( 'Remove', 'volley' ); ?>">
						<?php
							...
							echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf(
								...
								esc_html__( 'Remove this item', 'volley' ),
								esc_attr( $product_id ),
			...
			<td colspan="3" class="cart-coupon">
				...
				<span class="cart-coupon-inner">
					...
					<form class="checkout_coupon" method="post">
						...
						<input type="text" name="coupon_code" class="input-text" id="coupon_code" value="" placeholder="<?php esc_attr_e( 'Coupon code', 'volley' ); ?>" />
						<button type="submit" class="button apply_coupon" name="apply_coupon" value="<?php esc_attr_e( 'Apply Coupon', 'volley' ); ?>">
			...
			<td colspan="5" class="actions">
				...
				<div class="woo-actions-inner">
					...
					<a href="<?php echo esc_url( apply_filters( 'woocommerce_return_to_shop_redirect', wc_get_page_permalink( 'shop' ) ) ); ?>" class="button continue_shopping">
						<span><?php esc_html_e( 'Continue Shopping', 'volley' ); ?></span>
					</a>
					<button type="submit" class="button update_cart" name="update_cart" value="<?php esc_attr_e( 'Update cart', 'volley' ); ?>">
						<span><?php esc_html_e( 'Update cart', 'volley' ); ?></span>
					</button>
