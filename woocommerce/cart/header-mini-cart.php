...
<div class="ld-cart-head hidden">
	<span class="font-weight-bold text-uppercase ltr-sp-175"><?php esc_html_e( 'Your cart', 'volley' ); ?> <span class="color-primary"><?php echo esc_html( $order_count ) ?></span></span>
</div><!-- /.ld-cart-head -->
...
<div class="ld-cart-products">
				...
				if ( $_product && $_product->exists() && $cart_item['quantity'] > 0 && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
					...
					<div class="<?php echo esc_attr( apply_filters( 'woocommerce_mini_cart_item_class', 'ld-cart-product', $cart_item, $cart_item_key ) ); ?>">
						...
						echo apply_filters( 'woocommerce_cart_item_remove_link', sprintf(
							'<a href="%s" class="ld-cart-product-remove remove" title="%s" data-product_id="%s" data-cart_item_key="%s" data-product_sku="%s"><i class="icon-ion-ios-close"></i></a>',
							esc_url( wc_get_cart_remove_url( $cart_item_key ) ),
							__( 'Remove this item', 'volley' ),
							esc_attr( $product_id ),
	...
	<?php else : ?>
		...
		<div class="empty"><h3><?php esc_html_e( 'No products in the cart.', 'volley' ); ?></h3></div>
...
<div class="ld-cart-foot">
	<div class="ld-cart-total">
		<span class="ld-cart-total-label font-weight-bold text-uppercase ltr-sp-175"><?php esc_html_e( 'Subtotal', 'volley' ); ?></span>
	...
	<div class="ld-cart-button">
		<a href="<?php echo esc_url( WC()->cart->get_checkout_url() ); ?>" class="btn btn-xlg btn-solid text-uppercase ltr-sp-175">
			<span>
				<span class="btn-txt"><?php esc_html_e( 'Checkout', 'volley' ); ?></span>
				<span class="btn-icon"><i class="fa fa-angle-right"></i></span>
			</span>
		</a>
		<a href="<?php echo esc_url( wc_get_cart_url() ); ?>" class="btn btn-xlg btn-naked text-uppercase ltr-sp-175">
			<span>
				<span class="btn-txt"><?php esc_html_e( 'View Cart', 'volley' ); ?></span>
				<span class="btn-icon"><i class="fa fa-angle-right"></i></span>
			</span>
		</a>
	</div><!-- /.ld-cart-button -->
