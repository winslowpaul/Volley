...
/**
 * Class ThemeThreads_Vc_Templates_Panel_Editor
 * @since 1.0
 */
class ThemeThreads_Vc_Templates_Panel_Editor {
	/**
	 ...
	 * @var bool
	 */
	protected $themethreads_templates = false;
	/**
	 * @since 4.4
	...
	public function init() {
		...
		$enable_volley_collection = themethreads_helper()->get_option( 'enable-volley-collection' );
		if( 'off' === $enable_volley_collection ) {
			return;
		}		
	...
	public function addTemplatesTab( $data ) {
		$newCategory = array(
			'category'        => 'themethreads_templates',
			'category_name'   => esc_html__( 'Volley Collection', 'volley' ),
			'category_weight' => 1,
	...
	public function renderTemplateBlock( $category ) {
		...
		if ( 'themethreads_templates' === $category['category'] ) {

			$category['output'] = '<div class="vc_col-md-2 themethreads-sorting-container">';
			$category['output'] .= $this->get_template_categories();
			$category['output'] .= '</div>';


			$category['output'] .= '
			<div class="vc_column vc_col-md-10 themethreads-templates-container">
	...
	function renderTemplateWindow( $template_name, $template_data ) {
		...
		if ( 'themethreads_templates' === $template_data['type'] ) {
			return $this->renderTemplateWindowThemeThreadsTemplates( $template_name, $template_data );
		}
	 ...
	 * @return string
	 */
	public function renderTemplateWindowThemeThreadsTemplates( $template_name, $template_data ) {
		ob_start();
		...
		$preview_template_title = esc_attr( 'Preview template', 'volley' );
		$add_template_title = esc_attr( 'Add template', 'volley' );
...
HTML;
	...
	public function renderBackendTemplate() {
		...
		if ( ! isset( $template_id, $template_type ) || '' === $template_id || '' === $template_type ) {
			die( 'Error: Vc_ThemeThreads_Templates::renderBackendTemplate:1' );
		}
	 ...
	 * @param $templates
	 *
	 * vc_filter: vc_load_themethreads_templates_limit_total - total items to show
	 *
	 * @return array
	 */
	...
	public function getAllTemplates() {
		...
		$data = array();
		$themethreads_templates = $this->getTemplates();
		$category_templates = array();
		foreach ( $themethreads_templates as $template_id => $template_data ) {
			$category_templates[] = array(
				'unique_id' => $template_id,
				'name' => $template_data['name'],
				'new' => isset( $template_data['new'] ) ? $template_data['new'] : false,
				'type' => 'themethreads_templates',
	 ...
	 * To modify you should use add_filter('vc_load_themethreads_templates','your_custom_function');
	 ...
	 *          ...
	 *      );
	 * Also see filters 'vc_load_themethreads_templates_panels' and 'vc_load_themethreads_templates_welcome_block' to modify
	 * templates in panels tab and/or in welcome block. vc_filter: vc_load_themethreads_templates - filter to override
	...
	public function loadDefaultTemplates() {
		...
		if ( ! is_array( $this->themethreads_templates ) ) {
			$this->themethreads_templates = $this->allTemplates();
		}

		return $this->themethreads_templates;
	}
	...
	public function getDefaultTemplate( $template_index ) {
		...
		if ( ! is_numeric( $template_index ) || ! is_array( $this->themethreads_templates ) || ! isset( $this->themethreads_templates[ $template_index ] ) ) {
			return false;
		}

		return $this->themethreads_templates[ $template_index ];
	}
	...
	public function addDefaultTemplates( $data ) {
		if ( is_array( $data ) && ! empty( $data ) && isset( $data['name'], $data['content'] ) ) {
			if ( ! is_array( $this->themethreads_templates ) ) {
				$this->themethreads_templates = array();
			}
			$this->themethreads_templates[] = $data;
	...
	public function renderTemplateListItem( $template ) {
		$name = isset( $template['name'] ) ? esc_html( $template['name'] ) : esc_html( __( 'No title', 'volley' ) );
...
HTML;
		$output .= '<div class="themethreads-template-preview">';
		if ( $new ) { $output .= '<span class="themethreads-badge-new">New</span>'; }
		...
		$output .= '<span class="themethreads-template-categories">' . esc_html( $template_sort_name ) . '</span>';
		$output .= <<<HTML
						</div>
