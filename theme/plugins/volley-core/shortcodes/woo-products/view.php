...
// check
if( !themethreads_helper()->is_woocommerce_active() ) {
	return;
}
...
switch ( $orderby ) {
	...
	case 'popularity' :
		...
		add_filter('posts_clauses', 'themethreads_woocommerce_order_by_popularity_post_clauses');
	break;
	case 'rating' :
		// Sorting handled later though a hook
		add_filter('posts_clauses', 'themethreads_woocommerce_order_by_rating_post_clauses');
	break;
	case 'title' :
		$args['orderby'] = 'title';
		$args['order'] = $order == 'DESC' ? 'DESC' : 'ASC';
...
<div class="ld-bsp-carousel">
	<div class="carousel-container carousel-dots-style2">
		<div class="carousel-items row mx-0" data-threads-flickity='{ "imagesLoaded": true, "adaptiveHeight": true, "prevNextButtons": true, "navArrow": {
			...
			<?php
				...
				remove_filter('posts_clauses', 'themethreads_woocommerce_order_by_popularity_post_clauses');
				remove_filter('posts_clauses', 'themethreads_woocommerce_order_by_rating_post_clauses');
