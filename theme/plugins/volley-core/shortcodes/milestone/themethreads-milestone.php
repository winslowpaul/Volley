...
class LD_Milestone extends LD_Shortcode { 
	...
	public function __construct() {
		...
		$this->title       = esc_html__( 'Milestone box', 'volley-core' );
		$this->icon        = 'fa fa-list-ol';
		$this->description = esc_html__( 'Create milestone box.', 'volley-core' );
	...
	public function get_params() {
		...
		$params = array(
			...
			array(
				'type'       => 'textfield',
				'param_name' => 'date',
				'heading'    => esc_html__( 'Date/Time', 'volley-core' ),
	...
	protected function get_time() {
		...
		printf( '<div class="themethreads-milestone-time h3"><span>%s</span></div>', esc_html( $this->atts['date'] ) );
	...
	protected function generate_css() {
		...
		if( ! empty( $primary_color ) ) {
			$elements[ themethreads_implode( '%1$s .themethreads-milestone-time' ) ]['color'] = esc_attr( $primary_color );
		}
