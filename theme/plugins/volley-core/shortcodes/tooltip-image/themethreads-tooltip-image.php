...
class LD_Tooltiped_Image extends LD_Shortcode {
	...
	public function __construct() {
		...
		$this->title        = esc_html__( 'Image with Hotspots', 'volley-core' );
		$this->description  = esc_html__( 'Tooltips on the image', 'volley-core' );
	...
	protected function get_image() {
		...
		if( preg_match( '/^\d+$/', $this->atts['image'] ) ){
			$src = themethreads_get_image_src( $this->atts['image'] );
			$img_src = $src[0];
	...
	protected function generate_css() {
		...
		if( !empty( $primary_color ) ) {
			$elements[ themethreads_implode( '%1$s .info-box:before, %1$s .info-box:after' ) ]['background'] = $primary_color;
		}
