...
class LD_Masked_Image extends LD_Shortcode {
	...
	public function __construct() {
		...
		$this->title           = esc_html__( 'Masked Image', 'volley-core' );
		$this->description     = esc_html__( 'Add masked image', 'volley-core' );
	...
	public function get_params() {
		...
		$this->params = array(
			...
			array(
				'type'             => 'textfield',
				'param_name'       => 'bg_pos_x',
				'heading'          => esc_html__( 'Background Position X', 'volley-core' ),
				'description'      => esc_html__( 'Add Background position on axe X with px, for ex. 24px', 'volley-core' ),
				'edit_field_class' => 'vc_col-sm-6',
			),
			array(
				'type'             => 'textfield',
				'param_name'       => 'bg_pos_y',
				'heading'          => esc_html__( 'Background Position Y', 'volley-core' ),
				'description'      => esc_html__( 'Add Background position on axe Y with px, for ex. 24px', 'volley-core' ),
				'edit_field_class' => 'vc_col-sm-6',
			),
			array(
				'type'        => 'checkbox',
	...
	protected function generate_css() {
		...
		$elements[ themethreads_implode( '%1$s .clip-svg' ) ]['clip-path'] = 'url(#svg-' . $this->get_id() . ' )';
		$elements[ themethreads_implode( '%1$s .clip-svg' ) ]['-webkit-clip-path'] = 'url(#svg-' . $this->get_id() . ' )';
		
		if( !empty( $bg_pos_x ) ) {
			$elements[ themethreads_implode( '%1$s .clip-svg' ) ]['background-position-x'] = $bg_pos_x;	
		}
		if( !empty( $bg_pos_y ) ) {
			$elements[ themethreads_implode( '%1$s .clip-svg' ) ]['background-position-y'] = $bg_pos_y;
		}
