...
echo '<div class="themethreads-portfolio-list ' . $this->get_id() . '">';
	...
	if( 'masonry-creative' === $style ) {
		...
		if( 'yes' === $atts['show_filter'] ) {
			...
			printf( '<div id="%1$s" class="row themethreads-portfolio-list-row %1$s" data-themethreads-masonry="true" data-columns="%4$s" data-masonry-options=\'{ "filtersID": "#%2$s", "layoutMode": "masonry", "alignMid": true }\' %3$s>', $this->grid_id, $filter_id, $this->get_options(), $grid_columns );
			echo '<div class="col-md-4 col-sm-6 col-xs-12 grid-stamp creative-masonry-grid-stamp"></div>';
		}
		else {
			printf( '<div id="%1$s" class="row themethreads-portfolio-list-row %1$s" data-columns="%3$s" data-themethreads-masonry="true" data-masonry-options=\'{ "layoutMode": "masonry", "alignMid": true }\' %2$s>', $this->grid_id, $this->get_options(), $grid_columns );
			echo '<div class="col-md-4 col-sm-6 col-xs-12 grid-stamp creative-masonry-grid-stamp"></div>';
		}
	}
	else {
		...
		if( 'yes' === $atts['show_filter'] ) {
			$filter_located = locate_template( 'templates/portfolio/partial-filters.php' );
			include $filter_located;
			printf( '<div id="%1$s" class="row themethreads-portfolio-list-row %1$s" data-themethreads-masonry="true" data-masonry-options=\'{ "filtersID": "#%2$s" }\' %3$s>', $this->grid_id, $filter_id, $this->get_options() );
		}
		else {
			printf( '<div id="%1$s" class="row themethreads-portfolio-list-row %1$s" data-themethreads-masonry="true" %2$s>', $this->grid_id, $this->get_options() );
		}
	}
	...
	if( 'pagination' === $atts['pagination'] ) {
        ...
        $links = paginate_links( array(
			'type' => 'array',
			'prev_next' => true,
			'prev_text' => '<span aria-hidden="true">' . __( '<i class="fa fa-angle-left"></i>', 'volley-core' ) . '</span>',
			'next_text' => '<span aria-hidden="true">' . __( '<i class="fa fa-angle-right"></i>', 'volley-core' ) . '</span>'
		));
		if( !empty( $links ) ) {
			printf( '<div class="page-nav"><nav aria-label="' . esc_attr__( 'Page navigation', 'volley-core' ) . '"><ul class="pagination"><li>%s</li></ul></nav></div>', join( "</li>\n\t<li>", $links ) );
		}
	}
