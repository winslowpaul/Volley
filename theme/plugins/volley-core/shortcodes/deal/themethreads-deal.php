...
class LD_Deal extends LD_Shortcode {
	...
	public function __construct() {
		...
		$this->description = esc_html__( 'Create a banner', 'volley-core' );
	...
	public function get_params() {
		...
		$button = vc_map_integrate_shortcode( 'ld_button', 'bn_', esc_html__( 'Button', 'volley-core' ),
			array(
				'exclude' => array(
					'el_id',
					'el_class',
		...
		$general = array(
			...
			array(
				'type'       => 'textfield',
				'param_name' => 'info',
				'heading'    => esc_html__( 'Label/Additional Info', 'volley-core' ),
			),
			array(
		...
		$design = array(
			...
			array(
				...
				'description' => esc_html__( 'Pick a custom color for the title color', 'volley-core' ),
				'edit_field_class' => 'vc_col-sm-6',
			...
			array(
				...
				'description' => esc_html__( 'Pick a custom color for the label/subtitle background color', 'volley-core' ),
				'edit_field_class' => 'vc_col-sm-6',
				'group'       => esc_html__( 'Design Options', 'volley-core' ),
			)
	...
	protected function generate_css() {
		...
		if( !empty( $primary_color ) ) {
			$elements[ themethreads_implode( '%1$s .ld-bnr-subtitle,%1$s .ld-bnr-label:after' ) ]['background'] = esc_attr( $primary_color );
		}
		if( !empty( $color ) ) {
			$elements[ themethreads_implode( '%1$s .ld-bnr-title' ) ]['color'] = esc_attr( $color );
		}
