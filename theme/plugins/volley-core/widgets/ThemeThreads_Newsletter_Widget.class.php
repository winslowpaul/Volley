<?php
/**
* ThemeThreads Newslleter Widget
*
* @package Volley theme
*/
 
class ThemeThreads_Newsletter_Widget extends WP_Widget {

	function __construct() {
		$widget_ops = array( 'classname' => 'widget_subscribe', 'description' => esc_html__( "Display Newsletter Form", 'volley-core' ) );
		parent::__construct( 'themethreads-newsllter', esc_html__( 'ThemeThreads Themes: Newsletter Form', 'volley-core' ), $widget_ops);

		$this-> alt_option_name = 'widget_themethreads_subscribe';
	...
	function widget( $args, $instance ) {
		...
		$cache = wp_cache_get('widget_themethreads_subscribe', 'widget');
	...
	<?php
		...
		wp_cache_set('widget_themethreads_subscribe', $cache, 'widget');
	}
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset( $alloptions['widget_themethreads_subscribe'] ) ) {

			delete_option('widget_themethreads_subscribe');
	...
	function flush_widget_cache() {
		wp_cache_delete( 'widget_themethreads_subscribe', 'widget' );
	}
	...
	function form( $instance ) {
		...
		$forms = array_merge_recursive( array( esc_html__( 'Select', 'volley-core' ) => '' ) , array_flip( $this->get_mailpoet_forms() ) );
		
		$styles = array(
			'default' => esc_html__( 'Default', 'volley-core' ),
			'alt' => esc_html__( 'Style 1', 'volley-core'  ),
		);
		...
		<p>
			<label for="<?php echo esc_attr( $this->get_field_id( 'newsletter_id' ) ); ?>"><?php esc_html_e( 'Newsletter:', 'volley-core' ); ?></label>
