...
class ThemeThreadsDownload
{
	 ...
	 * @var string
	 */
	private $temp_folder_name = 'themethreads_temp';

	/**
	 * ThemeThreads Api url
	 *
	 * @access private
	 * @var string
	 */
	...
	{
		$this->core = $core;
		$this->api_url = $core['ThemeThreadsEnvato']->api_new_url;
	}

	public function run() {
		add_action( 'wp_ajax_themethreads_prepare_demo_package', array($this, 'ajax_prepare_demo'), 10, 1 );
	}
	...
	public function download($demo){
		$token = $this->core['ThemeThreadsCheck']->get_token();
		...
		$args = array(
			'timeout'    => 30,
			'user-agent' => 'themethreads-user-agent',
		);
	...
	public function ajax_prepare_demo() {
		...
		$is_valid = $this->core['ThemeThreadsCheck']->is_vaild();
		$ret = '';
		...
		if( !$download ) {
			$ret = '{"stat":0, "message":"Your server was unable to connect to ThemeThreads Themes API server. Please check with your hosting company if the connection to https://api.themethreadswp.com is blocked in the firewall or network setup."}';
			echo $ret;
			wp_die();
		}
		...
		if( isset($download['content-length']) && $download['content-length'] >= 1 ) {
			...
			$ret = '{"stat":0, "message":"Failed to download the demo package or the downloaded file is corrupted, Contact ThemeThreads Themes support !"}';
		}
		echo $ret;
		wp_die();
	}
