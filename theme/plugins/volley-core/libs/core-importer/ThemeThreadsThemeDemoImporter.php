...
class ThemeThreadsThemeDemoImporter extends ThemeThreadsImporter {
    /**
    ...
    public function __construct($core) {
		...
		$this->demo_files_path = $core['ThemeThreadsDownload']->temp_folder().DIRECTORY_SEPARATOR;
		if(function_exists('themethreads')) {
			$this->theme_option_name = themethreads()->get_option_name();
		}
    ...
    public function run() {
    	add_action( 'wp_ajax_themethreads_import_form', array($this, 'ajax_import'), 10, 1 );
    	add_action( 'wp_ajax_themethreads_import_theme_options', array($this, 'ajax_import_theme_options'), 10, 1 );
    	add_action( 'wp_ajax_themethreads_import_theme_widgets', array($this, 'ajax_import_theme_widgets'), 10, 1 );
    	add_action( 'wp_ajax_themethreads_set_home_page', array($this, 'ajax_set_home_page'), 10, 1 );
    	add_action( 'wp_ajax_themethreads_import_slider', array($this, 'import_slider'));
    	add_action( 'wp_ajax_themethreads_set_demo_content', array($this, 'ajax_set_demo_conten'));
    	add_action( 'wp_ajax_themethreads_import_media', array($this, 'import_media'));
    	add_action( 'wp_ajax_themethreads_require_plugins', array($this, 'ajax_get_require_plugins'));
    }
    ...
    public function import_revslider($data = ''){
    	...
    	if(class_exists('RevSliderSlider')) {
    		...
    		echo esc_html__( 'Failed to import slider data, Please make sure to install and activate slider revolution plugin first', 'volley-core' );
	...
	public function set_templates($demo) {
		...
		foreach ($templates as $option => $page) {
			...
			if($matches) {
				$_page = get_page_by_title( $page, OBJECT,  'themethreads-'.$matches[0]);
				if(class_exists('ThemeThreads_Theme_Options')) {
					$theme = new ThemeThreads_Theme_Options;
