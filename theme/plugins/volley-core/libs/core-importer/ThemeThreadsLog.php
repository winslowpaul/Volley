...
class ThemeThreadsLog
{
	...
	function __construct()
	{
		...
		$this->log_folder = 'themethreads_log';
		$this->imported = 0;

	}
	public function run() {
		add_action( 'wp_ajax_themethreads_progress_imported', array($this, 'getProgress'), 10, 1 );
		add_action( 'wp_ajax_themethreads_total_imported', array($this, 'getImported'), 10, 1 );
		add_action( 'wp_ajax_themethreads_reset_logs', array($this, 'resetFiles'), 10, 1 );
		add_action( 'admin_footer', array( $this, 'loadingTpl'), 10, 1 );
	}

	public function loadingTpl(){
		echo '
		  <div id="themethreads-demo-loader" class="threads-imp-popup-wrap">
			<div class="threads-imp-loader">			
