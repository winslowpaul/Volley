 ...
 */
function themethreads_get_recent_tweets($username, $consumer_key, $consumer_secret, $access_token, $access_token_secret, $cache_time = 3600) {
	...
	{
		require_once themethreads_addons()->plugin_dir() .'/extras/class/tmhOAuth.php';
		require_once themethreads_addons()->plugin_dir() .'/extras/class/tmhUtilities.php';
 ...
 * @return string
 */
function themethreads_get_linkedin_connections( $api_key, $api_secret ) {
 ...
 * @return null
 */
function themethreads_replace_in_tweets($tweet) {
	...
	if (!empty($tweet->entities->urls) || !empty($tweet->entities->hashtags) || !empty($tweet->entities->user_mentions)) {
		...
		$tweet_text = themethreads_replace_urls_with_html($tweet_text);
 ...
 * @return string
 */
function themethreads_replace_urls_with_html($text) {
	...
	return preg_replace_callback("&\\b$rexProtocol$rexDomain$rexPort$rexPath$rexQuery$rexFragment(?=[?.!,;:\"]?(\s|$))&", 'themethreads_replace_urls_with_html_callback', htmlspecialchars($text));
}
 ...
 * @return type
 */
function themethreads_replace_urls_with_html_callback($match) {
 ...
 * @return type
 */
function themethreads_time_elapsed_string($datetime, $full = false) {
	$now = new DateTime;
 ...
 * @return string
 */
function themethreads_get_remote_html( $url = '' ) {

	// Check for transient, if none, grab remote HTML file
	if ( false === ( $html = get_transient( 'themethreads_remote_html' ) ) ) {
		...
		set_transient( 'themethreads_remote_html', $data, 24 * HOUR_IN_SECONDS );
 ...
 * @return number
 */
function themethreads_get_twitter_followers( $screenName = '', $consumerKey= '', $consumerSecret= '' ) {
 ...
 * @return string
 */
function themethreads_format_num( $num, $precision = 2 ) {
 ...
 * @return string
 */
function themethreads_get_facebook_likes( $page_link, $facebook_access_token ){

	// Check for transient, if none, grab number of likes
	if ( false === ( $fb_likes = get_transient( 'themethreads_facebook_likes' ) ) ) {
		...
		set_transient( 'themethreads_facebook_likes', $data_likes, 24 * HOUR_IN_SECONDS );
 ...
 * @return string
 */
function themethreads_get_google_plus_followers( $page_id = '', $google_api_key = '' ) {

	// Check for transient, if none, grab number of followers
	if ( false === ( $gplus_followers = get_transient( 'themethreads_gplus_followers' ) ) ) {
		...
		set_transient( 'themethreads_gplus_followers', $data_circled, 24 * HOUR_IN_SECONDS );
 ...
 * @return string
 */
function themethreads_get_behance_followers( $page_link, $behance_api_key = '' ) {

	// Check for transient, if none, grab number of followers
	if ( false === ( $b_followers = get_transient( 'themethreads_behance_followers' ) ) ) {
		...
		set_transient( 'themethreads_behance_followers', $data_followers, 24 * HOUR_IN_SECONDS );
 ...
 * @return string
 */
function themethreads_get_dribbble_followers( $dribbble_access_token ) {

	// Check for transient, if none, grab number of followers
	if ( false === ( $drbbb_followers = get_transient( 'themethreads_dribbble_followers' ) ) ) {
		...
		set_transient( 'themethreads_dribbble_followers', $data_followers, 24 * HOUR_IN_SECONDS );
 ...
 * @return string
 */
function themethreads_get_pinterest_followers( $url ) {

	// Check for transient, if none, grab number of followers
	if ( false === ( $pinterest_followers = get_transient( 'themethreads_pinterest_followers' ) ) ) {
		...
		set_transient( 'themethreads_pinterest_followers', $meta_followers, 24 * HOUR_IN_SECONDS );
 ...
 * @return string
 */
function themethreads_get_instagram_followers( $url ) {

	// Check for transient, if none, grab number of followers
	if ( false === ( $instagram_followers = get_transient( 'themethreads_instagram_followers' ) ) ) {

		$raw = themethreads_get_remote_html( $url ); 
		...
		set_transient( 'themethreads_instagram_followers', $instagram_count, 24 * HOUR_IN_SECONDS );
...
/**
 * [themethreads_woo_share description]
 * @method themethreads_woo_share
 * @return [type] [description]
 */
function themethreads_woo_share( ) { 

	if ( ! class_exists( 'ReduxFramework' ) ) { 
		return;
	}	
	
	if( 'off' === themethreads_helper()->get_option( 'wc-share-enable' ) ) {
		return;
	}
...
<ul class="social-icon scheme-dark">
	...
	<li><a href="<?php echo esc_url( $pinterestURL ); ?>"><i class="fa fa-pinterest-p"></i></a></li>
</ul><!-- /.social-icon -->
<?php
}
add_action( 'woocommerce_share', 'themethreads_woo_share' );

if( ! function_exists( 'themethreads_portfolio_share' ) ) {
	/**
	 * [themethreads_portfolio_share description]
	 * @method themethreads_portfolio_share
	 * @return [type]                [description]
	 */
	function themethreads_portfolio_share( $post_type = 'post', $args = array() ) {
		
		if ( ! class_exists( 'ReduxFramework' ) ) { 
			return;
		}
		
		$option_name = str_replace( 'themethreads-', '', $post_type ) . '-social-box-';
