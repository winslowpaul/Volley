...
// Template Tags -------------------------------------------------------
function themethreads_likes_button( $args = array(), $post_id = 0 ) {

	// if post support post likes
	if( !post_type_supports( get_post_type( $post_id ), 'themethreads-post-likes' ) ) {
		echo __( 'Post type not support likes.', 'volley-core' );
		return;
	}

	echo ThemeThreads_Post_Like::instance()->get_likes_button( $args, $post_id );
}
...
/**
 * ThemeThreads_Post_Like Theme
 */
class ThemeThreads_Post_Like extends ThemeThreads_Base {

	/**
     * Hold an instance of ThemeThreads_Post_Like class.
     * @var ThemeThreads_Post_Like
     */
	 ...
	 * @var string
	 */
	protected $metakey = '_themethreads_likes_count';

	/**
	 * [$noncekey description]
	 * @var string
	 */
	protected $noncekey = 'themethreads-likes-nonce';

	/**
	 * Main ThemeThreads_Post_Like instance.
	 *
	 * @return ThemeThreads_Post_Like - Main instance.
	 */
    public static function instance() {

		if( null == self::$instance ) {
            self::$instance = new ThemeThreads_Post_Like();
        }
	...
	public function enqueue_scripts() {
		...
		wp_register_script( 'themethreads-likes', LD_ADDONS_URL . '/extensions/post-likes/themethreads-likes.js', array( 'jquery' ), '0.1', true );
	}
	...
	public function save_like() {
		...
		if ( ! wp_verify_nonce( $nonce, $this->noncekey ) ) {
			wp_send_json_error( esc_html__( 'Not permitted', 'volley-core' ) );
		}

		// Get post_id
		$post_id = isset( $_REQUEST['post_id'] ) && is_numeric( $_REQUEST['post_id'] ) ? $_REQUEST['post_id'] : '';
		if( ! $post_id ) {
			wp_send_json_error( esc_html__( 'No Post ID', 'volley-core' ) );
		}

		// if post support post likes
		if( !post_type_supports( get_post_type( $post_id ), 'themethreads-post-likes' ) ) {
			wp_send_json_error( esc_html__( 'Post type not support likes.', 'volley-core' ) );
		}

		// Base variables.
		$result = array( 'success' => true );
		$cookie_key = 'themethreads_post_like_' . $post_id;
	...
	public function get_likes_button( $args = array(), $post_id = 0 ) {
		...
		$defaults = array(
			...
			'format' => wp_kses_post( __( '<span class="post-likes-count">%s</span> Likes', 'volley-core' ) ),
		);
		...
		$out .= sprintf( '<%s class="%s">', $container, $classes );
			$out .= sprintf( '<a href="#" class="themethreads-post-like" data-id="%s" data-security="%s">', $post_id, $nonce );
...
ThemeThreads_Post_Like::instance();
