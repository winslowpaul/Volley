...
if ( ! class_exists( 'ReduxFramework_iconpicker' ) ) {
    class ReduxFramework_iconpicker {
        ...
        public function render() {
				...
				echo '<select data-id="' . $this->field['id'] . '" data-placeholder="' . $placeholder . '" name="' . $this->field['name'] . $this->field['name_suffix'] . '" class="redux-select-item themethreads-icon-picker ' . $this->field['class'] . '">';
				echo '<option value=""></option>';
				
				$this->field['options'] = apply_filters( 'themethreads_menu_iconpicker_icons', array() );
        ...
        function enqueue() {
			...
			wp_enqueue_style( 'themethreads-iconpicker', $url . '/css/jquery.fonticonpicker.min.css' );
			wp_enqueue_style( 'themethreads-iconpicker-theme', $url . '/themes/grey-theme/jquery.fonticonpicker.grey.min.css' );
			wp_enqueue_script( 'themethreads-iconpicker', $url . '/jquery.fonticonpicker.min.js' ,  array('jquery'), '1.0.0', true, false );
