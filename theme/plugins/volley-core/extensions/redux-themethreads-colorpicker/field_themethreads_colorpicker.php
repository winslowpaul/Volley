...
if ( ! class_exists( 'ReduxFramework_themethreads_colorpicker' ) ) {
    class ReduxFramework_themethreads_colorpicker {
        ...
        function enqueue() {
            ...
            if (!wp_script_is ( 'redux-field-themethreads-colorpicker-js' )) {

				$url = trailingslashit( plugin_dir_url( __FILE__ ) );

				wp_enqueue_style( 'ld-colorpicker', $url . 'themethreads-colorpicker.css' );
				wp_enqueue_script( 'ld-colorpicker', $url . 'grapick.min.js' ,  array('jquery'), '1.0.0', true );

                wp_enqueue_script(
                    'redux-field-themethreadscolorpicker-js',
                    $url . '/plugin.themethreadsColorPicker.min.js',
