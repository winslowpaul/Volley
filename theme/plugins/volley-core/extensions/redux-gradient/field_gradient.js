(function( $ ) {
	...
	redux.field_objects.gradient.init = function( selector, skipCheck) {
		...
		var $css = selector.find('.themethreads-gradient-css'),
			$field = selector.find('.regular-text'),
			value = $field.val(),
			element = selector.find('.themethreads-gradient'),
			gradientDir = element.siblings('.themethreads-gradient-direction');
		...
		gp.setColorPicker(function(handler) {
			...
			$(el).spectrum({
				...
				containerClassName: 'themethreads-gradient-picker-container',
				replacerClassName: 'themethreads-gradient-picker-replacer',
				change(color) {
		...
		function update() {
			...
			$(gp.el).siblings('.themethreads-gradient-css').val(gp.getSafeValue());
			$field.val(gp.getSafeValue());
			$(gp.el).siblings('.themethreads-gradient-preview').children('.themethreads-gradient-preview-inner').css('background-image', gp.getSafeValue());
		...
		// selector.find('.themethreads-gradient').icsge(options);
		// selector.find('.themethreads-gradient-css').on( 'change', function(){
