<?php
/**
* ThemeThreads Responsive Options
*/
...
*/
if( ! class_exists( 'ThemeThreads_Responsive_Css_Editor' ) ) {

	class ThemeThreads_Responsive_Css_Editor  {
		...
		function responsive_param( $settings, $value ) {
			...
			$label = isset( $settings['label'] ) ? $settings['label'] : esc_html__( 'Responsive Options', 'volley-core' );
			...
			foreach( $devices as $device ) {
				...
				$output .= '<h3 class="themethreads-responsive-css-heading '. $device .'" title="' . $device . '"><i class="' . $icons[ $device ] . '"></i></h3>';
		...
		public static function generate_css( $value, $id = '' ) {
			...
			$values = ThemeThreads_Responsive_Css_Editor::get_responsive_values( $value );
...
new ThemeThreads_Responsive_Css_Editor;