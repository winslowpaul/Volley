<?php
/**
* ThemeThreads Responsive Margin
*/
...
*/
if( ! class_exists( 'ThemeThreads_Responsive_Margin' ) ) {

	class ThemeThreads_Responsive_Margin  {
		...
		function responsive_param( $settings, $value ) {
			...
			$label = isset( $settings['label'] ) ? $settings['label'] : esc_html__( 'Responsive Margin', 'volley-core' );
			$values = $this->get_responsive_values( $value );
			$output = '<div class="themethreads-responsive-css-container vc_css-editor vc_row vc_ui-flex-row">';
			...
			foreach( $devices as $device ) {
				...
				$output .= '<div class="themethreads-main-responsive-wrapper ' . $active . '">';
				$output .= '<h3 class="themethreads-responsive-css-heading '. $device .'">' . $device  . '</h3>';
		...
		public static function generate_css( $value, $id = '' ) {
			...
			$values = ThemeThreads_Responsive_Margin::get_responsive_values( $value );
...
new ThemeThreads_Responsive_Margin;