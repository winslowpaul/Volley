...
if( !class_exists( 'ThemeThreads_Responsive_Param' ) ) {

	class ThemeThreads_Responsive_Param  {
		...
		function __construct() {
			...
			if ( function_exists( 'vc_add_shortcode_param' ) ) {
				vc_add_shortcode_param( 'themethreads_responsive', array( $this, 'responsive_param' ) );
			}
		...
		function responsive_param( $settings, $value ) {
			...
			$label = isset( $settings['label'] ) ? $settings['label'] : esc_html__( 'Responsive Parameters', 'volley-core' );
			...
			$output .= '<div class="themethreads-responsive-container vc_css-editor vc_row vc_ui-flex-row">';
				
			
			foreach( $devices as $device ) {
				
				$output .= '<div class="themethreads-main-responsive-wrapper">';
				$output .= '<h3 class="themethreads-responsive-css-heading '. $device .'"><i class="' . $icons[ $device ] . '"></i> ' . $attribute . '</h3>';
		...
		public static function generate_css( $css,  $value, $id = '' ) {
			...
			$values = ThemeThreads_Responsive_Param::get_responsive_values( $value );
...
new ThemeThreads_Responsive_Param;