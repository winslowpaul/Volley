...
class LD_Helper {
	...
	public function get_param( $id, $old ) {
		...
		if( ! isset( themethreads_addons()->params[$id] ) ) {
			_doing_it_wrong( 'get_param', wp_kses( sprintf( __( 'ID: <strong>%s</strong>, didn\'t exists in the system', 'volley-core' ), $id ), array( 'strong' => array() ) ), null );
		}

		$new = array_merge( themethreads_addons()->params[$id], $old );
		unset( $new['id'] );
	...
	public function add_param( $id, $param ) {
		...
		if( isset( themethreads_addons()->params[$id] ) ) {
			_doing_it_wrong( 'add_param', wp_kses( sprintf( __( 'ID: <strong>%s</strong>, already exists in the system', 'volley-core' ), $id ), array( 'strong' => array() ) ), null );
		}

		themethreads_addons()->params[$id] = $param;
	}
	...
	public function remove_param( $id ) {
		...
		if( ! isset( themethreads_addons()->params[$id] ) ) {
			_doing_it_wrong( 'remove_param', wp_kses( sprintf( __( 'ID: <strong>%s</strong>, didn\'t exists in the system', 'volley-core' ), $id ), array( 'strong' => array() ) ), null );
		}

		unset( themethreads_addons()->params[$id] );
	}
    ...
    */
    public function get_term_object( $term ) {
    	...
    	return array(
    		...
    		'group' => isset( $vc_taxonomies_types[ $term->taxonomy ], $vc_taxonomies_types[ $term->taxonomy ]->labels, $vc_taxonomies_types[ $term->taxonomy ]->labels->name ) ? $vc_taxonomies_types[ $term->taxonomy ]->labels->name : __( 'Taxonomies', 'volley-core' ),
    		);
    }
