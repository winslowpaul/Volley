...
class LD_Shortcode extends WPBakeryShortCode {
	...
	public function __construct() {
		...
		if( ! isset( $this->slug ) && empty( $this->slug ) ) {
			wp_die( esc_html__( 'Please define slug', 'volley-core' ), esc_html__( 'Variable Missing', 'volley-core' ) );
		}
	...
	protected function set_config() {
		...
		$shortcode = array(
			...
			'name' => $this->title,
			'params' => $this->params,
			'category' => ! empty( $this->category ) ? $this->category : esc_html__( 'ThemeThreads', 'volley-core' )
		);
	...
	protected function scripts( $args = array() ) {
		...
		wp_dequeue_script( 'themethreads-theme' );
	...
	public function prepare_params() {
		...
		foreach( $this->params as $id => &$param ) {
			...
			if( empty( $param['description'] ) && isset( $param['heading'] ) && 'subheading' != $param['type'] ) {
				$prefix = esc_html__( 'Select ', 'volley-core' );

				if( in_array( $param['type'], array( 'textarea', 'textfield', 'textarea_html' ) ) ) {
					$prefix = esc_html__( 'Enter  ', 'volley-core' );
				}
	...
	public function render( $atts, $content = '' ) {
		...
		if ( !$located ) {
			trigger_error( sprintf( esc_html__( 'Template file is missing for `%s` shortcode. Make sure you have `%s` file in your theme folder or default folder.', 'volley-core' ), $this->title, 'view.php' ) );
			return;
		}
	...
	public function dynamic_css_parser( $id, $elements ) {
		...
		$check = apply_filters( 'themethreads_dinamic_css_output', true );
	...
	public function get_wordpress_data( $type = false, $args = array() ) {
		...
		$data = array(
			esc_html__( 'Select option', 'volley-core' ) => ''
		);
...
class LD_ShortcodeContainer extends WPBakeryShortCodesContainer {
	...
	public function mainHtmlBlockParams( $width, $i ) {
		...
		return 'data-element_type="' . $this->settings['base'] . '" class="themethreads-content-holder wpb_' . $this->settings['base'] . ' ' . $sortable . ' wpb_content_holder vc_shortcodes_container"' . $this->customAdminBlockParams();
	}
		...
		for ( $i = 0; $i < count( $width ); $i ++ ) {
			...
			$output .= '<div ' . $this->mainHtmlBlockParams( $width, $i ) . '>';
				...
				$output .= '<div class="wpb_element_wrapper">';
					...
					$output .= '<div class="themethreads-param-holder">';
